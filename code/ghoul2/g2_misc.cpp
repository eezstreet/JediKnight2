...
void R_TransformEachSurface( const mdxmSurface_t *surface, vec3_t scale, CMiniHeap *G2VertSpace, int *TransformedVertsArray,CBoneCache *boneCache) 
{
	...
	if (!TransformedVerts)
	{
		Com_Error(ERR_DROP, "Ran out of transform space for Ghoul2 Models. Adjust MiniHeapSize in sv_init.cpp.\n");
	}
...
//	assert(DotProduct(basis2,basis2)>.0001f);
	...
	const float c=cos((double)0);//theta
	const float s=sin((double)0);//theta
