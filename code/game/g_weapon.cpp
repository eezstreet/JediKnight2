...
//	}
	...
	VectorMA( start, shotRange, wpForward, end );
	...
	if ( render_impact )
	{
		if ( tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage )
		{
			...
			{//hehe
				G_Damage( traceEnt, ent, ent, wpForward, tr.endpos, 3, DAMAGE_DEATH_KNOCKBACK, MOD_DISRUPTOR, hitLoc );
			}
			else
			{
				G_Damage( traceEnt, ent, ent, wpForward, tr.endpos, damage, DAMAGE_DEATH_KNOCKBACK, MOD_DISRUPTOR, hitLoc );
			}
		}
		else 
		{
	...
	for ( dist = 0; dist < shotDist; dist += 64 )
	{
		...
		VectorMA( start, dist, wpForward, spot );
		AddSightEvent( ent, spot, 256, AEL_DISCOVERED, 50 );
	}
	VectorMA( start, shotDist-4, wpForward, spot );
	AddSightEvent( ent, spot, 256, AEL_DISCOVERED, 50 );
}
...
//---------------------------------------------------------
{
	...
	else
	{
		...
		AngleVectors( ent->client->renderInfo.eyeAngles, wpForward, NULL, NULL );
		...
		if ( !hitDodged )
		{
			if ( render_impact )
			{
					...
					|| traceEnt->s.eType == ET_MOVER )
				{
					...
					if ( traceEnt && traceEnt->client && traceEnt->client->NPC_class == CLASS_GALAKMECH )
					{//hehe
						G_Damage( traceEnt, ent, ent, wpForward, tr.endpos, 10, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC, fullCharge ? MOD_SNIPER : MOD_DISRUPTOR, hitLoc );			
						break;
					}
					G_Damage( traceEnt, ent, ent, wpForward, tr.endpos, damage, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC, fullCharge ? MOD_SNIPER : MOD_DISRUPTOR, hitLoc );
				}
				else 
				{
...
//---------------------------------------------------------
{
	gentity_t	*missile = CreateMissile( muzzle, wpForward, FLECHETTE_MINE_VEL, 10000, ent, qtrue );
...
{
	// Chucking a new one
	AngleVectors( self->client->ps.viewangles, wpForward, wpVright, wpUp );
	CalcMuzzlePoint( self, wpForward, wpVright, wpUp, wpMuzzle, 0 );
	VectorNormalize( wpForward );
	VectorMA( wpMuzzle, -4, wpForward, wpMuzzle );
...
//---------------------------------------------------------
{
	...
	{
		WP_DropDetPack( ent, wpMuzzle, wpForward );
...
//---------------------------------------------------------
{
	...
	if ( laserTrap )
	{
		...
		VectorScale( wpForward, LT_VELOCITY, laserTrap->s.pos.trDelta );
...
void WP_FireStunBaton( gentity_t *ent, qboolean alt_fire )
{
	...
	VectorMA( start, STUN_BATON_RANGE, wpForward, end );
...
//		G_Sound( tr_ent, G_SoundIndex( va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) );
		...
		G_Damage( tr_ent, ent, ent, wpForward, tr.endpos, STUN_BATON_DAMAGE, DAMAGE_NO_KNOCKBACK, MOD_MELEE );
	}
	...
	{
		G_Damage( tr_ent, ent, ent, wpForward, tr.endpos, 999, DAMAGE_NO_KNOCKBACK, MOD_MELEE ); // smash that puppy
	}
}
...
//---------------------------------------------------------
{
	...
	VectorMA( wpMuzzle, range, wpForward, end );
	...
	{
		G_Sound( tr_ent, G_SoundIndex( va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) );
		G_Damage( tr_ent, ent, ent, wpForward, tr.endpos, damage, DAMAGE_NO_KNOCKBACK, MOD_MELEE );
	}
}
...
//---------------------------------------------------------
{
	...
	if ( ent->s.weapon == WP_DISRUPTOR && alt_fire )
	{
		if ( ent->NPC )
		{
			...
			AngleVectors( ent->lastAngles, wpForward, wpVright, wpUp );
		}
	}
	else if ( ent->s.weapon == WP_ATST_SIDE || ent->s.weapon == WP_ATST_MAIN ) 
	{
		...
		{//player driving an AT-ST
			...
			AngleVectors( ent->client->ps.viewangles, wpForward, wpVright, wpUp );
			//CalcMuzzlePoint( ent, wpForward, wpVright, up, muzzle, 0 );
		}
		else if ( !ent->enemy )
		{//an NPC with no enemy to auto-aim at
			VectorCopy( ent->client->renderInfo.muzzleDir, wpForward );
		}
		else
		{//NPC, auto-aim at enemy
			...
			AngleVectors (angleToEnemy1, wpForward, wpVright, wpUp);
		}
	} 
	...
	CalcMuzzlePoint ( ent, wpForward, wpVright, wpUp, wpMuzzle , 0);
